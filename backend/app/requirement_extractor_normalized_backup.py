...
def _detect_alternatives(text: str) -> List[Dict[str, Any]]:
    ...
    This is used AFTER Gemini so that obvious logical mistakes
    can be corrected even if the model expanded alternatives.
    """
    ...
    cloud_pattern = re.search(
        r"(?:at least one|one of)?\s*"
        r"(?:AWS|Amazon Web Services)"
        r"\s*(?:,|/|or|and/or)\s*"
        r"(?:GCP|Google Cloud(?: Platform)?|Azure)"
        r"(?:\s*(?:,|/|or|and/or)\s*(?:AWS|GCP|Google Cloud(?: Platform)?|Azure))*",
        text,
        flags=re.IGNORECASE,
    )

    has_aws = bool(
        re.search(r"\bAWS\b", text, re.IGNORECASE)
    )
        ...
        groups.append(
            {
                ...
                "logic": "ANY_OF",
                "options": options,
                "evidence": "at least one major cloud provider",
                "source": "rule",
            }
        )
            ...
            groups.append(
                {
                    ...
                    "evidence": "Kafka, Kinesis, or equivalent",
                    "source": "rule",
                }
            )

    # --------------------------------------------------------
    # Docker + Kubernetes are BOTH required when the JD explicitly
    # connects them with "and".
    # --------------------------------------------------------
        ...
        groups.append(
            {
                ...
                "logic": "ALL_OF",
                "options": ["Docker", "Kubernetes"],
                "evidence": "Docker and Kubernetes",
                "source": "rule",
            }
        )
            ...
            groups.append(
                {
                    ...
                    "evidence": "Prometheus/Grafana or equivalent",
                    "source": "rule",
                }
            )
        ...
        groups.append(
            {
                ...
                "evidence": "distributed tracing with OpenTelemetry or equivalent",
                "source": "rule",
            }
        )
